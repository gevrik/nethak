...
CHAR_DATA *find_keeper(CHAR_DATA *ch) {
	...
	pShop = NULL;
	for (keeper = ch->in_room->first_person; keeper; keeper
			= keeper->next_in_room)
	...
	if (time_info.hour > pShop->close_hour) {
		do_say(keeper, "> come back tomorrow");
		return NULL;
	}
	...
	if (fBuy) {
		...
		for (itype = 0; itype < MAX_TRADE; itype++) {
			if (obj->item_type == pShop->buy_type[itype]) {
				cost = (int) (obj->cost * UMIN( (pShop->profit_buy-1),
						pShop->profit_sell+profitmod)) / 100;
				break;
			}
		}
		for (obj2 = keeper->first_carrying; obj2; obj2 = obj2->next_content) {
			if (obj->pIndexData == obj2->pIndexData && obj->item_type
					!= ITEM_RESOURCE && obj->item_type != ITEM_CRYSTAL
					&& obj->item_type != ITEM_FOOD && obj->item_type
					!= ITEM_RARE_METAL) {
				cost /= (obj2->count + 1);
				break;
			}
...
void do_buy(CHAR_DATA *ch, char *argument) {
	...
	/* in case of different shop types */
	{
		...
		if (is_number(arg)) {
			...
			argument = one_argument(argument, arg);
			if (noi > mnoi) {
				act(AT_TELL, "> $n tells you 'I do not sell that many items at"
		...
		if (cost <= 0 || !can_see_obj(ch, obj)) {
			act(AT_TELL, "> $n tells you 'I do not sell that -- try 'list'.'",
		...
		if (!IS_OBJ_STAT( obj, ITEM_INVENTORY ) && (noi > 1)) {
			...
			act(AT_TELL,
					"> $n tells you 'I do not have enough of those in stock"
		...
		if (ch->gold < cost) {
			act(AT_TELL, "> $n tells you 'You cannot afford to buy $p.'",
		...
		if (IS_SET(obj->extra_flags, ITEM_PROTOTYPE) && IS_IMMORTAL( ch )) {
			act(
					AT_TELL,
					"> $n tells you 'This is a only a prototype!  I cannot sell you that...'",
					keeper, NULL, ch, TO_VICT);
			ch->reply = keeper;
			return;
		}
		...
		if (ch->carry_weight + (get_obj_weight(obj) * noi) + (noi > 1 ? 2 : 0)
				> can_carry_w(ch)) {
		...
		if (noi == 1) {
			...
			sprintf(arg, "> $n buys %d $p%s", noi, (obj->short_descr[strlen(
					obj->short_descr) - 1] == 's' ? "" : "s"));
			act(AT_ACTION, arg, ch, obj, NULL, TO_ROOM);
			sprintf(arg, "> you buy %d $p%s", noi, (obj->short_descr[strlen(
					obj->short_descr) - 1] == 's' ? "" : "s"));
			act(AT_ACTION, arg, ch, obj, NULL, TO_CHAR);
			act(AT_ACTION, "> $N puts them into a bag and hands it to you", ch,
					NULL, keeper, TO_CHAR);
		}
...
void do_list(CHAR_DATA *ch, char *argument) {
	...
	for (obj = keeper->last_carrying; obj; obj = obj->prev_content) {
		...
		if (obj->wear_loc == WEAR_NONE && can_see_obj(ch, obj)) {
			oref++;
			if ((cost = get_cost(ch, keeper, obj, TRUE)) > 0 && (arg[0] == '\0'
				...
				ch_printf(
						ch,
						"[%5d] {%3d} " MXPTAG ("list '%.*s' '%s'") "%s" MXPTAG ("/list") "\n\r",
						cost, oref, i, obj->name, obj->short_descr,
						obj->short_descr);
			}
		}
	}
	...
	if (!found) {
		if (arg[0] == '\0')
			...
			send_to_char("> you cannot buy that here\n\r", ch);
	}
	return;
}
...
void do_sell(CHAR_DATA *ch, char *argument) {
	...
	if ((obj = get_obj_carry(ch, arg)) == NULL) {
		act(AT_TELL, "> $n tells you 'You do not have that item.'", keeper,
				NULL, ch, TO_VICT);
		ch->reply = keeper;
		return;
	}
	...
	if (clan) {
		...
		int clanfundsmin = 50000;

		if (clan->funds < clanfundsmin) {
			act(AT_TELL, "> organization currently not buying", keeper, obj,
					ch, TO_VICT);
			return;
		}

			if (cost > clan->funds) {
				act(AT_TELL, "> not enough organization funds", keeper, obj,
						ch, TO_VICT);
				return;
			}
			...
			sprintf(buf, "> you sell $p for %d credit%s", cost, cost == 1 ? ""
					: "s");
			act(AT_ACTION, buf, ch, obj, NULL, TO_CHAR);
			int sellchance = number_range(1, 100);
 ...
 */
void repair_one_obj(CHAR_DATA *ch, CHAR_DATA *keeper, OBJ_DATA *obj, char *arg,
		int maxgold, char *fixstr, char*fixstr2) {
	char buf[MAX_STRING_LENGTH];
	int cost;

	if (!can_drop_obj(ch, obj))
		ch_printf(ch, "> you cannot let go of %s\n\r", obj->name);
	else if ((cost = get_repaircost(keeper, obj)) < 0) {
		if (cost != -2)
			act(AT_TELL,
					"> $n tells you, 'Sorry, I cannot do anything with $p.'",
					keeper, obj, ch, TO_VICT);
		else
			act(AT_TELL, "> $n tells you, '$p looks fine to me!'", keeper, obj,
					ch, TO_VICT);
	}
	/* "repair all" gets a 10% surcharge - Gorog */

	else if ((cost = strcmp("all", arg) ? cost : 11 * cost / 10) > ch->gold) {
		sprintf(buf, "> $N tells you, 'It will cost %d credit%s to %s %s...'",
				cost, cost == 1 ? "" : "s", fixstr, obj->name);
		act(AT_TELL, buf, ch, NULL, keeper, TO_CHAR);
		act(AT_TELL, "> $N tells you, 'Which I see you cannot afford.'", ch,
				NULL, keeper, TO_CHAR);
	} else {
		sprintf(buf, "> $n gives $p to $N, who quickly %s it", fixstr2);
		act(AT_ACTION, buf, ch, obj, keeper, TO_ROOM);
		sprintf(buf, "> $N charges you %d credit%s to %s $p", cost,
				cost == 1 ? "" : "s", fixstr);
...
void do_repair(CHAR_DATA *ch, char *argument) {
	...
	if (!strcmp(argument, "all")) {
		for (obj = ch->first_carrying; obj; obj = obj->next_content) {
			if (obj->wear_loc == WEAR_NONE && can_see_obj(ch, obj)
					&& (obj->item_type == ITEM_ARMOR || obj->item_type
							== ITEM_WEAPON || obj->item_type == ITEM_DEVICE))
				repair_one_obj(ch, keeper, obj, argument, maxgold, fixstr,
						fixstr2);
		}
		return;
	}
...
void appraise_all(CHAR_DATA *ch, CHAR_DATA *keeper, char *fixstr) {
	...
	if (total > 0) {
		send_to_char("\n\r", ch);
		sprintf(buf, "> $N tells you, 'It will cost %d credit%s in total.'",
				total, cost == 1 ? "" : "s");
		act(AT_TELL, buf, ch, NULL, keeper, TO_CHAR);
		strcpy(pbuf,
...
void do_appraise(CHAR_DATA *ch, char *argument) {
	...
	sprintf(buf, "> $N tells you, 'It will cost %d credit%s to %s that...'",
...
void do_makeshop(CHAR_DATA *ch, char *argument) {
	...
	if (!argument || argument[0] == '\0') {
		send_to_char("> syntax: makeshop <mobvnum>\n\r", ch);
		return;
	}
...
void do_shopset(CHAR_DATA *ch, char *argument) {
	...
	if (arg1[0] == '\0' || arg2[0] == '\0') {
		send_to_char("> syntax: shopset <mob vnum> <field> value\n\r", ch);
		send_to_char("\n\rField being one of:\n\r", ch);
		send_to_char(
...
void do_shopstat(CHAR_DATA *ch, char *argument) {
	...
	if (argument[0] == '\0') {
		send_to_char("> syntax: shopstat <keeper vnum>\n\r", ch);
		return;
	}
	...
	ch_printf(ch, "buy0 [%s]  buy1 [%s]  buy2 [%s]  buy3 [%s]  buy4 [%s]\n\r",
			...
			o_types[shop->buy_type[2]], o_types[shop->buy_type[3]],
...
void do_makerepair(CHAR_DATA *ch, char *argument) {
	...
	if (!argument || argument[0] == '\0') {
		send_to_char("> syntax: makerepair <mobvnum>\n\r", ch);
		return;
	}
...
void do_repairset(CHAR_DATA *ch, char *argument) {
	...
	if (arg1[0] == '\0' || arg2[0] == '\0') {
		send_to_char("> syntax: repairset <mob vnum> <field> value\n\r", ch);
		send_to_char("\n\rField being one of:\n\r", ch);
...
void do_repairstat(CHAR_DATA *ch, char *argument) {
	...
	if (argument[0] == '\0') {
		send_to_char("> syntax: repairstat <keeper vnum>\n\r", ch);
		return;
	}
	...
	ch_printf(ch, "fix0 [%s]  fix1 [%s]  fix2 [%s]\n\r",
			o_types[repair->fix_type[0]], o_types[repair->fix_type[1]],
