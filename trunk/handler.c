...
/* spell cast upon wear/removal of an object	-Thoric */
	...
	&&   skill->type == SKILL_SPELL )
	   if ( (retcode=(*skill->spell_fun) ( mod, ch->top_level, ch, ch )) == rCHAR_DIED
	   ||   char_died(ch) )
	      return;
	break;
...
/* skill apply types	-Thoric */
	...
	if ( !IS_NPC(ch) && ch->pcdata->learned[gsn_pick_lock]> 0)
	  ch->pcdata->learned[gsn_pick_lock] =
		UMAX( 1, ch->pcdata->learned[gsn_pick_lock] + mod );
	break;
    case APPLY_BACKSTAB:
	if ( !IS_NPC(ch) && ch->pcdata->learned[gsn_backstab]> 0)
	  ch->pcdata->learned[gsn_backstab] =
		UMAX( 1, ch->pcdata->learned[gsn_backstab] + mod );
	break;
    case APPLY_DODGE:
	...
	if ( !IS_NPC(ch) && ch->pcdata->learned[gsn_disarm]> 0 )
	  ch->pcdata->learned[gsn_disarm] =
		UMAX( 1, ch->pcdata->learned[gsn_disarm] + mod );
	break;
    case APPLY_KICK:
...
OBJ_DATA *find_obj( CHAR_DATA *ch, char *argument, bool carryonly )
{
	...
	if ( !obj )
		...
		"> you see nothing like that beneath $p" :
		"> you see nothing like that in $p",
...
char *cyber_bit_name(int cyber_flags)
{
    ...
    buf[0] = '\0';
    if (cyber_flags & CYBER_COMM                ) strcat(buf, " internal comm unit\n\r");
    if (cyber_flags & CYBER_EYES                ) strcat(buf, " infrared eyes\n\r");
    if (cyber_flags & CYBER_LEGS                ) strcat(buf, " leg enhancements\n\r");
    if (cyber_flags & CYBER_CHEST               ) strcat(buf, " chest plating\n\r");
    if (cyber_flags & CYBER_REFLEXES    ) strcat(buf, " augmented reflexes\n\r");
    if (cyber_flags & CYBER_MIND                ) strcat(buf, " internal computer\n\r");
    if (cyber_flags & CYBER_STRENGTH    ) strcat(buf, " augmented muscles\n\r");
        if (cyber_flags & CYBER_REACTOR         ) strcat(buf, " cyberjack\n\r");
        if (cyber_flags & CYBER_STERILE     ) strcat(buf, " Cyber Sterialation\n\r");
    return ( buf[0] != '\0' ) ? buf+1 : "none";
}
