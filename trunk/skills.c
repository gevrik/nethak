...
void do_steal( CHAR_DATA *ch, char *argument )
{
	...
	{
		send_to_char( "> you cannot steal from ICE\n\r", ch );
		return;
	}
	...
	if ( victim->substate != SUB_NONE )
	{
		send_to_char( "> you cannot steal from them right now\n\r", ch );
		return;
	}
	...
	{
		send_to_char( "> you cannot steal from that user\n\r", ch );
		return;
	}
			...
			||   percent > ( IS_NPC(ch) ? 90 : ch->pcdata->learned[gsn_steal] ) )
	{
		...
		sprintf( buf, "> %s is a thief", ch->name );
		do_yell( victim, buf );
			...
			||   !str_cmp( arg1, "money"  ) )
	{
		...
		if ( amount <= 0 )
		{
			send_to_char( "> you could not get any credits\n\r", ch );
			learn_from_failure( ch, gsn_steal );
			return;
		}
		...
		ch->gold     += amount;
		victim->gold -= amount;
		ch_printf( ch, "> you received %d credits\n\r", amount );
	...
	if ( ( obj = get_obj_carry( victim, arg1 ) ) == NULL )
	{
		if ( victim->position <= POS_SLEEPING )
		{
			if ( ( obj = get_obj_wear( victim, arg1 ) ) != NULL )
			{
				if ( (obj_next=get_eq_char(victim, obj->wear_loc)) != obj )
				{
					...
					send_to_char( "> you will have to steal that first\n\r", ch );
					learn_from_failure( ch, gsn_steal );
					return;
				}
				else
		...
		send_to_char( "> you cannot seem to find it\n\r", ch );
		learn_from_failure( ch, gsn_steal );
		return;
	}
	...
	{
		send_to_char( "> you cannot manage to pry it away\n\r", ch );
		learn_from_failure( ch, gsn_steal );
		return;
	}
...
void do_rescue( CHAR_DATA *ch, char *argument )
{
	...
	{
		send_to_char( "> does not need your help\n\r", ch );
		return;
	}
	...
	{
		send_to_char( "> one would imagine THEY do not need your help.\r\n", ch );
		return;
	}
...
void do_disarm( CHAR_DATA *ch, char *argument )
{
	...
	{
		send_to_char( "> you do not know how to disarm opponents\n\r", ch );
		return;
	}
...
void do_pick( CHAR_DATA *ch, char *argument )
{
	...
	if ( ( obj = get_obj_here( ch, arg ) ) != NULL )
	{
		...
		{ send_to_char( "> you cannot pick that\n\r", ch ); return; }
	...
	if ( ( ship = ship_in_room(ch->in_room, arg ) ) != NULL )
	{
		...
		{
			send_to_char("&R> what would be the point of that\n\r",ch);
			return;
		}

		if ( ship->shipstate != SHIP_DOCKED && ship->shipstate != SHIP_DISABLED )
		{
			send_to_char( "&R> that deck has already started to disconnect",ch);
			return;
		}
...
void do_aid( CHAR_DATA *ch, char *argument )
{
	...
	{
		act( AT_PLAIN, "> $N does not need your help", ch, NULL, victim,
				TO_CHAR);
		return;
	}
...
void do_dismount( CHAR_DATA *ch, char *argument )
{
	...
	if ( IS_NPC(ch) || number_percent( ) < ch->pcdata->learned[gsn_mount] )
	{
		...
		act( AT_SKILL, "> $n dismounts you", ch, NULL, victim, TO_VICT );
	...
	{
		act( AT_SKILL, "> you fall off while dismounting $N", ch, NULL, victim, TO_CHAR );
...
void do_poison_weapon( CHAR_DATA *ch, char *argument )
{
	...
	if ( ch->fighting )
	{
		send_to_char( "> not possible while in combat\n\r", ch );
		return;
	}
	if ( ms_find_obj(ch) )
		return;
	...
	{
		send_to_char( "> &Ryou do not have that weapon&w\n\r",      ch );
		return;
	}
	if ( obj->item_type != ITEM_WEAPON )
	{
		send_to_char( "> &Rthat item is not a weapon&w\n\r",        ch );
		return;
	}
	if ( IS_OBJ_STAT( obj, ITEM_POISONED ) )
	{
		send_to_char( "> &Rthat module is already poisoned&w\n\r",  ch );
		return;
	}
	...
	if ( !pobj )
	{
		send_to_char( "> &Ryou need some viral code&w\n\r", ch );
		return;
	}
...
//	}
	...
	{
		send_to_char("> you are not concentrated enough to do this\n\r", ch );
		return;
	}
	...
	//act(AT_BLUE, "> the remainder of the viral code eats through $p", ch, wobj, NULL, TO_CHAR );
...
void do_circle( CHAR_DATA *ch, char *argument )
{
	...
	if ( ch->mount )
	{
		send_to_char( "> you cannot circle while mounted\n\r", ch );
		return;
	}
	...
	if ( !ch->fighting )
	{
		send_to_char( "> you cannot circle when you are not fighting\n\r", ch);
		return;
	}

	if ( !victim->fighting )
	{
		send_to_char( "> you cannot circle around a person who is not fighting\n\r", ch );
		return;
	}

	if ( victim->num_fighting < 2 )
	{
		act( AT_PLAIN, "> you cannot circle around them without a distraction",
				ch, NULL, victim, TO_CHAR );
		return;
	}
...
void do_berserk( CHAR_DATA *ch, char *argument )
{
	...
	{
		send_to_char( "> you could not build up enough rage\n\r", ch);
...
void do_scan( CHAR_DATA *ch, char *argument )
{
	...
	{
		act( AT_GREY, "> you cannot see any further $t", ch, dir_name[dir], NULL, TO_CHAR );
		return;
	}
