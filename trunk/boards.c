...
void do_mailroom(CHAR_DATA *ch, char *argument)
{
    ...
    {
	send_to_char("> you can only use mail commands in a mail node\n\r", ch);
	return;
    }
    else
    {
...
void do_note( CHAR_DATA *ch, char *arg_passed, bool IS_MAIL )
{
	...
	if ( !can_read( ch, board ) )
	{
	    ...
	    if (first_list < 1)
	    {
		send_to_char( "> you cannot read a message before 1\n\r", ch);
		return;
	    }
	}
	...
	*/
	...
	if (!arg_passed || arg_passed[0] == '\0')
	{
	    send_to_char("> please specify a recipient\n\r", ch);
	    return;
	}
	if ( ( paper = get_eq_char(ch, WEAR_HOLD) ) == NULL
	||     paper->item_type != ITEM_PAPER )
	...
	if ( !can_read( ch, board ) ) 
	{
	    send_to_char( "> you cannot make any sense of what is posted here - let alone remove anything\n\r", ch );
	    return;
	}
...
void do_bstat( CHAR_DATA *ch, char *argument )
{
    ...
    if ( arg[0] == '\0' )
    {
	send_to_char( "> syntax: bstat <board filename>\n\r", ch );
	return;
    }
