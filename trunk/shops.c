...
int get_cost( CHAR_DATA *ch, CHAR_DATA *keeper, OBJ_DATA *obj, bool fBuy )
{
	...
	{
	    if ( obj->pIndexData == obj2->pIndexData && obj->item_type != ITEM_RESOURCE 
...
void do_buy( CHAR_DATA *ch, char *argument )
{
	...
	if ( !IS_OBJ_STAT( obj, ITEM_INVENTORY ) && ( noi > 1 ) )
	{
	    ...
	    act( AT_TELL, "> $n tells you 'I do not have enough of those in stock"
	...
	if ( IS_SET(obj->extra_flags, ITEM_PROTOTYPE) 
             && IS_IMMORTAL( ch ) )
	{
	    act( AT_TELL, "> $n tells you 'This is a only a prototype!  I cannot sell you that...'", 
		keeper, NULL, ch, TO_VICT );
      	    ch->reply = keeper;
	    return;
	}
...
void do_sell( CHAR_DATA *ch, char *argument )
{
    ...
    {
	act( AT_TELL, "> $n tells you 'You do not have that item.'",
		keeper, NULL, ch, TO_VICT );
	ch->reply = keeper;
	return;
    }
    ...
    clanfundsmin = 100000;
   	 ...
   	 {
		act( AT_TELL, "> organization currently not buying", keeper, obj, ch, TO_VICT );
		return;
   	 }
    ...
    if ( cost > keeper->gold )
    {
    	...
    	if ( cost > clan->funds )
   	 {
		act( AT_TELL, "> not enough organization funds", keeper, obj, ch, TO_VICT );
		return;
   	 }
                 ...
                 char *arg, int maxgold, char *fixstr, char*fixstr2 )
{
   ...
   {
       if (cost != -2)
       act( AT_TELL, "> $n tells you, 'Sorry, I cannot do anything with $p.'", 
            keeper, obj, ch, TO_VICT );
       else
...
void do_appraise( CHAR_DATA *ch, char *argument )
{
    ...
    {
      if (cost != -2)
	act( AT_TELL, "> $n tells you, 'Sorry, I cannot do anything with $p.'", keeper, obj, ch, TO_VICT );
      else
	act( AT_TELL, "> $n tells you, '$p looks fine to me!'", keeper, obj, ch, TO_VICT );
      return;
    }
