...
#define PREP_START  31924   /* vnum of first prep room */
#define PREP_END    31924   /* vnum of last prep room */
#define ARENA_START number_range( 31938, 31941)    /* vnum of first real arena room*/
#define ARENA_END   31941   /* vnum of last real arena room*/
...
void do_bet(CHAR_DATA *ch, char *argument)
 {
   ...
   if(arg[0]=='\0')
   {
      send_to_char("syntax: bet <player> <amount>\r\n",ch);
      return;
   }
   else if(!in_start_arena && !ppl_challenged)
   {
      send_to_char("> the arena is not yet open. please wait...\r\n", ch);
      return;
   }
   else if(ppl_in_arena)
   {
      send_to_char("> the arena has already started. no more bets\r\n", ch);
      return;
   }
   else if (!(ch->betted_on = get_char_world_ooc(ch, arg)))
      send_to_char("> no such runner in cyberspace", ch);
   ...
   else
      {
         ...
         if(newbet == 0)
         {
            send_to_char("> invalid bet\r\n", ch);
            return;
         }
         if (newbet > ch->gold)
         {
            send_to_char("> insufficient credits\n\r",ch);
            return;
         }
         if(newbet > 500)
         {
            send_to_char("> amount too high [must be less than 500]\r\n", ch);
            return;
         }
...
void do_chaos(CHAR_DATA *ch, char *argument)
{
 ...
 /*Usage: chaos lo hi start_delay cost/lev length*/
  ...
  if(hi_lim >= 200)
  {
     send_to_char("> please choose a hi_lim under 200\r\n", ch);
     return;
  }
  ...
  for (d = first_descriptor; d; d = d->next)
  {
...

     {
       ...
       if (IS_SET(i->in_room->room_flags, ROOM_ARENA))
       {
          ...
          i->move = i->max_move;
          i->challenged = NULL;
          i->opponent = NULL;  // VERSION 1.3 UPGRADE
...
int num_in_arena()
{
	...
	for (d = first_descriptor; d; d = d->next)
	{
		if( d->connected == CON_PLAYING )
		{
			...
			{
                     /*
                      * This is what allows an Immortal to be in the arena, keep in
                      * mind that it should always be ONE level above your Immortal
                      * max_level  -Diablo 2005
                      */
                     if( d->character->top_level <= 111 ) // VERSION 1.3 UPGRADE
                     num++;
			}
		}
	}
...
void do_challenge(CHAR_DATA *ch, char *argument)
{
 ...
 victim->opponent = ch; // VERSION 1.3 UPGRADE
 ch->opponent = victim; // VERSION 1.3 UPGRADE
}
