...
const struct map_info_type map_info[] =
{
    ...
    { SECT_BREEDING,     "^w&g", "!^x&w", "",   "breeding" },
    { SECT_BRIDGE,       "^x&Y", "#^x&w", "",   "bridge" },
    { SECT_CAMP,         "^g&W", "@^x&w", "",   "camp" },
    { SECT_CHASM,        "^x&z", "@^x&w", "",   "chasm" },
    { SECT_CORRIDOR,     "^w&x", " ^x&w", "",   "corridor" },
    { SECT_ELEVATOR,     "^x&W", "E^x&w", "",   "elevator" },
	{ SECT_FACTORY,  	 "^b&W", "$^x&w", "",   "factory" },
    { SECT_FARM,         "^c&C", "F^x&w", "",   "farm" },
    { SECT_HALL,         "^w&x", "H^x&w", "",   "hall" },
    { SECT_PLATFORM,     "^p&x", "P^x&w", "",   "platform" },
    { SECT_TUNNEL,       "^w&z", "#^x&w", "",   "tunnel" },
    { SECT_PRISON,		 "^r&x", "P^x&w", "",   "prison" },
	{ SECT_RAMP,  		 "^w&z", "R^x&w", "",   "ramp" },
	{ SECT_RUINS,  	     "^p&W", "R^x&w", "",   "ruins" },
	{ SECT_SETTLEMENT,   "^w&z", "S^x&w", "",   "settlement" },
    ...
    if (pbuf>buf)
    {
        ...
        if (last && pbuf[last]!='\n' && pbuf[last]!='\r')
        {
            ...
            *pbuf++='\r';
            if (currline<MAP_Y)
                pret += sprintf(pret, "%s   ", map[currline]);
            else if (currline==MAP_Y || (currline==MAP_Y+1 && blen<=term_width-
15))
                strcpy(pret, "              "), pret+=14;
                ++currline;
        }
 ...
 //   }
    ...
    if ( size != 7 )
    {
        size = IS_IMMORTAL( ch ) ? size :
            get_curr_int( ch )/2 +
            ( ( !IS_NPC( ch ) ) ? ch->pcdata->learned[gsn_spacecraft]/25 : 0 );
        if ( size % 2  == 0 )
            size +=1;
