...
/* from space.c */
/* ADDED FOR NEUROMANCER MUD */
void	write_ship_list();

char *  const   p_flags	 [] =
{
"nocap", "hidden", "explorable", "shut", "nopedit",
"noinvade", "notarget", "arena","p8", "p9", "p10", "p11", "p12", "p13",
...
"shipyard", "barracks", "r16", "no_edit", "arena", "coding",
"plr_home", "empty_home", "publicio", "agent", "nofloor", "pawn", "supply",
...
char * const    cyber_flags [] =
{
"comm", "eyes", "leg", "chest", "reflexes", "mind", "strength", "reactor","sterial"
};
...
int get_cyberflag( char *flag )
{
    int x;

    for ( x = 0; x < 32; x++ )
      if ( !str_cmp( flag, cyber_flags[x] ) )
        return x;
    return -1;
}
...
*/
	...
	if ( !argument || argument[0] == '\0' )
	{
	   ...
	   send_to_char( "Usage: redit flags <flag> [flag]...\n\r", ch );
           ...
           send_to_char( "nofloor, publicio, arena\n\r", ch );
	...
	if ( !argument || argument[0] == '\0' )
	{
	   ...
	   send_to_char( "Usage: redit sector <value>\n\r", ch );
           send_to_char( "\n\rSector Values:\n\r", ch );
           send_to_char( "0:dark, 1:default, 2:field, 3:forest, 4:hills, 5:mountain, 6:water_swim\n\r", ch );
