...
char * const sect_names[SECT_MAX][2] = { { "in a room", "inside" }, {
		"a default node", "default nodes" }, { "in a field", "fields" }, {
		"in a forest", "forests" }, { "hill", "hills" }, { "on a mountain",
		"mountains" }, { "in the water", "waters" }, { "in rough water",
		"waters" }, { "underwater", "underwaters" }, { "in the air", "air" }, {
		"in a desert", "deserts" }, { "somewhere", "unknown" }, {
		"ocean floor", "ocean floor" }, { "underground", "underground" }, {
		"scrub", "scrub" }, { "rocky terrain", "rocky" }, { "savanna",
		"savanna" }, { "tundra", "tundra" }, { "glacier", "glacial" }, {
		"rainforest", "rainforest" }, { "jungle", "jungle" }, { "swamp",
		"swamp" }, { "wetlands", "wetlands" }, { "brush", "brush" }, {
		"steppe", "steppe" }, { "farmland", "farmland" }, { "volcano",
		"volcanic" } };
...
bool will_fall(CHAR_DATA *ch, int fall) {
	if (IS_SET( ch->in_room->room_flags, ROOM_NOFLOOR ) && CAN_GO(ch, DIR_DOWN)
			&& (!IS_AFFECTED( ch, AFF_FLYING ) || (ch->mount
...
ch_ret move_char(CHAR_DATA *ch, EXIT_DATA *pexit, int fall) {
	...
	if (!IS_NPC( ch ))
		if (IS_DRUNK( ch, 2 ) && (ch->position != POS_SHOVE) && (ch->position
				!= POS_DRAG))
			drunk = TRUE;
		...
		if (!IS_SET( pexit->exit_info, EX_SECRET )
			...
			} else
			{
				...
				if( !strcmp(location->owner, ch->name) )
				{
					...
					send_to_char( "&R> you step through the codegate&w\n\r", ch );
					act(AT_ACTION, "> $n steps through the $d\n\r", ch, NULL, pexit->keyword,
							TO_ROOM);
					do_look( ch, "auto" );
	...
	if (!fall && IS_AFFECTED(ch, AFF_CHARM) && ch->master && in_room
			== ch->master->in_room) {
	...
	if (!fall && !IS_NPC(ch)) {
		...
		if (in_room->sector_type == SECT_AIR || to_room->sector_type
				== SECT_AIR || IS_SET( pexit->exit_info, EX_FLY )) {
		...
		if (in_room->sector_type == SECT_WATER_NOSWIM || to_room->sector_type
				== SECT_WATER_NOSWIM) {
			bool found;
	...
	if (!fall) {
		...
		for (fch = from_room->first_person; fch && (count < chars); fch
				= nextinroom) {
	...
	if (!will_fall(ch, fall) && fall > 0) {
		if (!IS_AFFECTED( ch, AFF_FLOATING ) || (ch->mount
				&& !IS_AFFECTED( ch->mount, AFF_FLOATING ))) {
			set_char_color(AT_HURT, ch);
			send_to_char("> you hit the ground\n\r", ch);
			WAIT_STATE( ch, 20 );
	...
	if ((pexit = find_door(ch, arg, TRUE)) != NULL) {
		...
		if (!IS_SET(pexit->exit_info, EX_SECRET) || (pexit->keyword
			...
			act(AT_ACTION, "> you open the $d\n\r", ch, NULL, pexit->keyword,
					TO_CHAR);
			if ((pexit_rev = pexit->rexit) != NULL && pexit_rev->to_room
					== ch->in_room) {
				CHAR_DATA *rch;

				for (rch = pexit->to_room->first_person; rch; rch
						= rch->next_in_room)
...
void do_bashdoor(CHAR_DATA *ch, char *argument) {
	...
	if ((pexit = find_door(ch, arg, FALSE)) != NULL) {
		...
		if (!IS_SET( pexit->exit_info, EX_BASHPROOF ) && ch->move >= 15
			...
			act(AT_SKILL, "> $n bashes against the $d but it holds strong", ch,
					NULL, keyword, TO_ROOM);
			damage(ch, ch, (ch->max_hit / 20) + 10, gsn_bashdoor);
			learn_from_failure(ch, gsn_bashdoor);
		}
	} else {
		act(AT_SKILL, "> you bash against the wall but it does not budge", ch,
				NULL, NULL, TO_CHAR);
		act(AT_SKILL, "> $n bashes against the wall but it holds strong", ch,
				NULL, NULL, TO_ROOM);
		damage(ch, ch, (ch->max_hit / 20) + 10, gsn_bashdoor);
		learn_from_failure(ch, gsn_bashdoor);
	}
	if (!char_died(ch))
		for (gch = ch->in_room->first_person; gch; gch = gch->next_in_room) {
			if (IS_AWAKE( gch ) && !gch->fighting && (IS_NPC( gch )
					&& !IS_AFFECTED( gch, AFF_CHARM )) && (ch->top_level
					- gch->top_level <= 4) && number_bits(2) == 0)
...
void do_sleep(CHAR_DATA *ch, char *argument) {
	...
	case POS_RESTING:
		if (ch->mental_state > 30 && (number_percent() + 10) < ch->mental_state) {
			send_to_char(
					"> you just cannot seem to calm yourself down enough to sleep\n\r",
					ch);
			act(
					AT_ACTION,
					"> $n closes $s eyes for a few moments but just cannot seem to go to sleep\n\r",
					ch, NULL, NULL, TO_ROOM);
			return;
		}
		send_to_char("> you close your eyes and drift into slumber\n\r", ch);
		act(AT_ACTION,
				"> $n closes $s eyes and drifts into a deep slumber\n\r", ch,
				NULL, NULL, TO_ROOM);
...
void do_wake(CHAR_DATA *ch, char *argument) {
	...
	if (IS_AFFECTED(victim, AFF_SLEEP) || victim->position < POS_SLEEPING) {
		act(AT_PLAIN, "> you cannot seem to wake $M!", ch, NULL, victim,
				TO_CHAR);
		return;
	}
