...
if ( !str_cmp( arg2, "survey" ) )
    {
	if ( IS_NPC(victim) )
	{
	    progbug("MpMset: can't set npc survey", ch);
	    return;
	}
	if ( victim->pcdata->learned[gsn_survey] > 0 )
	{
	progbug("MpMset: already has skillsoft", ch);
	return;
	}
	victim->pcdata->learned[gsn_survey] = 30;
	return;
	
    }

if ( !str_cmp( arg2, "coding" ) )
    {
	if ( IS_NPC(victim) )
	{
	    progbug("MpMset: can't set npc coding", ch);
	    return;
	}
	if ( victim->pcdata->learned[gsn_bridge] > 0 )
	{
	progbug("MpMset: already has skillsoft", ch);
	return;
	}
	victim->pcdata->learned[gsn_bridge] = 30;
	victim->pcdata->learned[gsn_landscape] = 30;
	victim->pcdata->learned[gsn_construction] = 30;
	return;
    }

if ( !str_cmp( arg2, "makeblaster" ) )
    {
	if ( IS_NPC(victim) )
	{
	    progbug("MpMset: can't set npc coding", ch);
	...
	if ( victim->pcdata->learned[gsn_makeblaster] > 0 )
	{
	progbug("MpMset: already has skillsoft", ch);
	return;
	}
	victim->pcdata->learned[gsn_makeblaster] = 20;
	return;
    }


if ( !str_cmp( arg2, "makedefmod" ) )
    {
	if ( IS_NPC(victim) )
	{
	    progbug("MpMset: can't set npc coding", ch);
	...
	if ( victim->pcdata->learned[gsn_makearmor] > 0 )
	{
	progbug("MpMset: already has skillsoft", ch);
	return;
	}
	victim->pcdata->learned[gsn_makearmor] = 20;
	return;
    }

if ( !str_cmp( arg2, "makeoffmod" ) )
    {
	if ( IS_NPC(victim) )
	{
	    progbug("MpMset: can't set npc coding", ch);
	...
	if ( victim->pcdata->learned[gsn_makeblade] > 0 )
	{
	progbug("MpMset: already has skillsoft", ch);
	return;
	}
	victim->pcdata->learned[gsn_makeblade] = 20;
	return;
    }

if ( !str_cmp( arg2, "makefw" ) )
    {
	if ( IS_NPC(victim) )
	{
	    progbug("MpMset: can't set npc coding", ch);
	...
	if ( victim->pcdata->learned[gsn_makeshield] > 0 )
	{
	progbug("MpMset: already has skillsoft", ch);
	return;
	}
...
if ( !str_cmp( arg2, "inquire" ) )
	...
	if ( victim->pcdata->learned[gsn_inquire] > 0 )
	{
	progbug("MpMset: already has skillsoft", ch);
	return;
	}
	victim->pcdata->learned[gsn_inquire] = 10;
	return;
    }

if ( !str_cmp( arg2, "slicebank" ) )
    {
	if ( IS_NPC(victim) )
	{
	    progbug("MpMset: can't set npc coding", ch);
	...
	if ( victim->pcdata->learned[gsn_slicebank] > 0 )
	{
	progbug("MpMset: already has skillsoft", ch);
	return;
	}
...
if ( !str_cmp( arg2, "climb" ) )
	...
	if ( victim->pcdata->learned[gsn_climb] > 0 )
	{
	progbug("MpMset: already has skillsoft", ch);
	return;
	}
	victim->pcdata->learned[gsn_climb] = 10;
	return;
    }
...
if ( !str_cmp( arg2, "backstab" ) )
	...
	if ( victim->pcdata->learned[gsn_backstab] > 0 )
	{
	progbug("MpMset: already has skillsoft", ch);
	return;
	}
	victim->pcdata->learned[gsn_backstab] = 10;
	return;
    }

if ( !str_cmp( arg2, "berserk" ) )
    {
	if ( IS_NPC(victim) )
	{
	    progbug("MpMset: can't set npc coding", ch);
	...
	if ( victim->pcdata->learned[gsn_berserk] > 0 )
	{
	progbug("MpMset: already has skillsoft", ch);
	return;
	}
	victim->pcdata->learned[gsn_berserk] = 10;
	return;
    }
