...
void do_setstarsystem( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char( "\n\rField being one of:\n\r", ch );
	send_to_char( "name flags filename\n\r", ch );
    ...
    if( !strcmp( arg2, "flags" ) )
    {
        if( argument[0] == '\0' )
	{
	   send_to_char("USAGE: setregion <region> flags <flag>\n\r", ch );
...
void do_showstarsystem( CHAR_DATA *ch, char *argument )
{
   ...
   if( !IS_IMMORTAL( ch ) && ( IS_SET( starsystem->flags, PLANET_HIDDEN ) || IS_SET( starsystem->flags, PLANET_EXPLORABLE) ) )
   {
	send_to_char( "No such starsystem.\n\r", ch );
	return;
   }
...
void do_starsystems( CHAR_DATA *ch, char *argument )
{
    ...
    for ( starsystem = first_starsystem; starsystem; starsystem = starsystem->next )
    	...
    	if( !IS_SET( starsystem->flags, PLANET_HIDDEN ) && !IS_SET( starsystem->flags, PLANET_EXPLORABLE ) )
...
void do_transship( CHAR_DATA *ch ,char *argument )
{
	...
	if ( arg1[0] == '\0' )
	{
		send_to_char( "Usage: transship <deck>\n\r", ch );
		send_to_char( "Usage: transship <deck> <vnum/zone>\n\r", ch );
		return;
	}
