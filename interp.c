...
bool check_pos( CHAR_DATA *ch, sh_int position )
{
	...
	case POS_RESTING:
	    send_to_char( "> you feel too relaxed\n\r", ch);
	    break;

	case POS_SITTING:
	    send_to_char( "> you cannot do that sitting down\n\r", ch);
	    break;
...
void interpret( CHAR_DATA *ch, char *argument )
{
	...
	if ( (IS_AFFECTED(ch, AFF_HIDE) && !IS_NPC(ch) && !nifty_is_name("backstab", argument))
			&& (IS_AFFECTED(ch, AFF_HIDE) && !IS_NPC(ch) && !nifty_is_name("bs", argument))){
...
bool check_social( CHAR_DATA *ch, char *command, char *argument )
{
    ...
    {
    case POS_DEAD:
	send_to_char( "> lie still - you are flatlined\n\r", ch );
	return TRUE;
